...
- **Secret-backed params are redacted before being stored, and re-resolved live on resume.** If a param's name is declared as a `secret:` key in the task's own `permissions.env` *and* its value matches that live secrets-chain entry, it's replaced with a redaction placeholder before `params` is written to `runs.resume_params` (tracked in `resume_params_redacted_fields`, mirroring `input_redacted_fields` -- [dicode-core#886](https://github.com/dicode-ayo/dicode-core/pull/886)). On resume, the value is restored by re-resolving that secret's *current* value from the secrets chain, never read back from the stored placeholder -- so a secret rotated between suspend and resume changes what the continuation runs with, and revoking the task's permission to it in that window fails the resume closed rather than falling back to a stale value. `GET /api/runs/{runID}` never returns the raw `resume_params` blob, only which fields were redacted.
