...
To be clear, this pattern-only exclusion is the *only* docker/podman limitation here: named `secret:`, `from:`, and `value:` entries in `permissions.env` are resolved and merged into the container environment like any other runtime, with a resolved value winning over a literal `docker.env_vars` value on a name collision, and resolved secret values are redacted from streamed container logs. See [Example: Injecting a secret as a container env var](/examples/docker-task#example-3-injecting-a-secret-as-a-container-env-var) for a worked example.
