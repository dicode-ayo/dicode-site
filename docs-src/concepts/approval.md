...
Each entry's `path`, `kind` (`file`, `symlink`, or `missing`), `size`, and `hash` are the same file inventory `GET /api/tasks/{id}` implies — no file content is ever read or rendered, only names, sizes, and hashes.
