...
| `test_task` | Runs the task's sibling test file (`task.test.ts` / `task.test.js` / `task.test.py`) for Deno/Python tasks, or builds and runs the Dockerfile's `test` build stage for Docker/Podman tasks (which have no sibling test file), and returns pass/fail counts and output. Refused for a task the approval gate is still holding pending — the test runs with full host permissions, so a pending task is turned away here exactly as it would be on a fire. Gated on `permissions.dicode.tasks_test` — the same flag also gates the REST endpoint `POST /api/tasks/{id}/test`, reachable with the same Bearer token, so the two share one gate. | `dicode.tasks.test()` |
