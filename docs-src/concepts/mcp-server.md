...
The MCP server exposes twelve tools. Three cover the MCP-exposed task catalog and are always available; `list_sources` stays a structured hint pointing at the dicode HTTP API; the remaining eight are **capability-gated** — each one appears only when the caller's token was granted the matching `permissions.dicode` capability. An undeclared capability makes the tool invisible rather than a permission error the caller discovers by calling it (dicode-core [#746](https://github.com/dicode-ayo/dicode-core/pull/746)).
...
| `dicode_set_dev_mode` | Toggles dev mode on a source (local-path or clone-mode). Returns `{ok, dev_root_path, clone_path}` — previously just `{ok: true}`, which left a caller holding a clone it had no way to locate. | `dicode.sources.set_dev_mode()` | `sources_set_dev_mode` |
...
| `dicode_git_commit_push` | Commits and pushes staged changes from a dev-mode clone. Commit author is defaulted by the daemon; the model can't set `allow_main` or the clone's `run_id` — see dicode-core#746's "What the model is not handed". | `dicode.git.commit_push()` | `git_commit_push` |
...
- `get_task` / `run_task` require the task to declare `permissions.dicode.list_tasks: true` (for `get_task`) and the target task ID in `permissions.dicode.tasks` (for `run_task`, or `["*"]` for any task).
