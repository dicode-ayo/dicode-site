...
This minted token is **not** a full-access credential — it's [scoped](./mcp-server.md#ephemeral-per-run-mcp-tokens) 1:1 to whatever `permissions.dicode` this task's own `task.yaml` declares. `get_task` requires `permissions.dicode.list_tasks: true`; `run_task` requires the target task ID in `permissions.dicode.tasks` (or `["*"]`); each capability-gated `dicode_*` tool (`dicode_list_tasks`, `dicode_get_runs`, `dicode_test_task`, `dicode_set_dev_mode`, `dicode_get_run_input`, `dicode_pin_run_input`/`dicode_unpin_run_input`, `dicode_replay_run`, `dicode_git_commit_push` — see the [tool surface table](./mcp-server.md#tool-surface)) requires its own matching capability. The shipped `buildin/ai-agent-claude-cli` `task.yaml` declares **no** `permissions.dicode` block at all, so out of the box the token Claude receives can't call `get_task`/`run_task` or any `dicode_*` tool — Claude only sees the always-allowed `list_sources` hint. An undeclared capability makes its tool invisible from `tools/list` rather than a permission error Claude discovers by calling it (dicode-core [#746](https://github.com/dicode-ayo/dicode-core/pull/746)). To let this agent actually list, test, or trigger dicode tasks over MCP, add a `permissions.dicode` block to its spec (typically via a taskset override layer) rather than assuming MCP access implies dicode-task access.
