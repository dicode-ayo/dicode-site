...
Beyond task-backed Tools, `buildin/ai-agent`'s own tool-calling loop can offer the model a second kind of tool: **built-ins**, backed directly by the agent's own `dicode.*` SDK calls rather than by firing another task. Unlike a Tool, a built-in isn't a file under `tasks/**` — it's wired into `ai-agent/task.ts` itself, and (dicode-core [#746](https://github.com/dicode-ayo/dicode-core/pull/746)) each one is offered to the model only when the running task's own `permissions.dicode` grants the matching capability. An undeclared capability makes the built-in invisible to the model — not a permission error it discovers by calling it.

| Built-in tool | Backed by | Requires `permissions.dicode.*` |
|---|---|---|
| `dicode_list_tasks` | `dicode.list_tasks()` | `list_tasks: true` |
| `dicode_get_runs` | `dicode.get_runs()` | `get_runs: true` |
| `dicode_test_task` | `dicode.tasks.test()` | `tasks_test: true` |
| `dicode_set_dev_mode` | `dicode.sources.set_dev_mode()`, clone-mode only | `sources_set_dev_mode: true` |
| `dicode_get_run_input` | `dicode.runs.get_input()` | `runs_get_input: true` |
| `dicode_pin_run_input` / `dicode_unpin_run_input` | `dicode.runs.pin_input()` / `dicode.runs.unpin_input()` | `runs_pin_input: true` / `runs_unpin_input: true` |
| `dicode_replay_run` | `dicode.runs.replay()` | `runs_replay: true` |
| `dicode_git_commit_push` | `dicode.git.commit_push()` | `git_commit_push: true` |

This is what actually powers the authoring loop described under [AI Task Authoring](#ai-task-authoring) below: `buildin/task-create` and `buildin/auto-fix` are `ai-agent` presets that declare a richer `permissions.dicode` block than the generic base, so they alone are offered the rest of this toolset — `dicode_test_task` lets an authoring session run the task it just wrote, and `dicode_git_commit_push` lets it commit and push the result. The generic base's own `task.yaml` already declares `list_tasks: true` (it's what backs the existing "agent discovers every registered task" behavior described above), so `buildin/ai-agent` and `buildin/dicodai` *are* offered `dicode_list_tasks` out of the box — they just don't gain the other eight built-ins without a preset-level `permissions.dicode` addition.

A few arguments are deliberately withheld from the model even when a built-in is offered, per #746's own "What the model is not handed": `dicode_set_dev_mode` only ever clones (no `local_path`, which would let the caller redirect what the daemon loads as tasks) and the clone's `run_id` is fixed to the calling run, not model-settable; `dicode_git_commit_push` has no `allow_main` argument — pushing to `main`/`master` isn't the model's call, and `branch_prefix` comes from the agent's own config. Secrets, crypto, audit, and the run-input retention sweep's own admin calls (`dicode.runs.list_expired()` / `dicode.runs.delete_input()`) have no built-in at all — those stay with the task process, not the model driving it.

This built-in mechanism is entirely separate from the `/mcp` JSON-RPC surface described in [MCP Server](./mcp-server.md) — it's wired into `ai-agent`'s own OpenAI-compatible chat loop, not into `buildin/mcp`. `buildin/ai-agent-claude-cli` (below) reaches dicode exclusively through the real `/mcp` server via `mcp__dicode`, so it does not gain any of these built-ins.

Also new: dicode-core [#738](https://github.com/dicode-ayo/dicode-core/pull/738) added `buildin/write-task-file`, a registered task (a Tool, not a built-in) that lets `buildin/task-create` actually write a task's files to disk during an authoring turn. Its writable root is bound via `permissions.env`, not a model-settable param, and confined to a task's own directory under the AI-scratch source — not an arbitrary dev-clone, closing a credential-exfiltration path a clone-rooted write would otherwise open (a git-backed source's `taskset.yaml` can carry a git ref whose `auth.token_env` the resolver reads from the daemon's own environment). Per-session confinement — restricting a write to *this session's own* task rather than any task under the AI-scratch source — remains open, per #738's "What was deliberately left behind" notes.
...
An **authoring session** is a server-side draft that holds proposed changes to one task's files — `task.yaml`, the script itself, and its test counterpart — before they are committed to the source git repo. A freshly AI-scaffolded task gets `task.yaml` + `task.ts` (dicode-core [#771](https://github.com/dicode-ayo/dicode-core/pull/771)); a hand-authored task, or an existing task reopened for editing, keeps whatever extension it already has — `task.ts` / `task.js` / `task.py`, with a matching `task.test.ts` / `task.test.js` / `task.test.py` where one exists. Sessions decouple the "write files" step from the "commit to git" step, giving you (or the AI) the opportunity to review a live diff in the WebUI and iterate before anything is permanent.
