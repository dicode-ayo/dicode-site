...
This is what actually powers the authoring loop described under [AI Task Authoring](#ai-task-authoring) below: `buildin/task-create` and `buildin/auto-fix` are `ai-agent` presets that declare a richer `permissions.dicode` block than the generic base, so they alone are offered this toolset — `dicode_test_task` lets an authoring session run the task it just wrote, and `dicode_git_commit_push` lets it commit and push the result. The base `buildin/ai-agent` and `buildin/dicodai` presets declare none of these capabilities beyond what [Security notes](#security-notes) already covers, so they're offered no built-ins at all.
