...
This minted token is **not** a full-access credential — it's [scoped](./mcp-server.md#ephemeral-per-run-mcp-tokens) 1:1 to whatever `permissions.dicode` this task's own `task.yaml` declares. `get_task` requires `permissions.dicode.list_tasks: true` and `run_task` requires the target task ID in `permissions.dicode.tasks` (or `["*"]`) — both stay visible in `tools/list` regardless and are denied at call time if the capability's missing. Each capability-gated `dicode_*` tool (`dicode_list_tasks`, `dicode_get_runs`, `dicode_test_task`, `dicode_set_dev_mode`, `dicode_get_run_input`, `dicode_pin_run_input`/`dicode_unpin_run_input`, `dicode_replay_run`, `dicode_git_commit_push` — see the [tool surface table](./mcp-server.md#tool-surface)) works differently: an undeclared capability makes that tool invisible from `tools/list` entirely, per dicode-core [#746](https://github.com/dicode-ayo/dicode-core/pull/746). The shipped `buildin/ai-agent-claude-cli` `task.yaml` declares **no** `permissions.dicode` block at all, so out of the box Claude sees `get_task`/`run_task` in its toolset but every call to either is denied, none of the nine `dicode_*` tools appear at all, and only the always-allowed `list_sources` hint is left fully usable. To let this agent actually list, test, or trigger dicode tasks over MCP, add a `permissions.dicode` block to its spec (typically via a taskset override layer) rather than assuming MCP access implies dicode-task access.
...
- **Partially governed tool access (security).** `claude -p` print mode actually runs with Claude's full default toolset (Read/Write/Edit/Bash/etc.) — it is not tool-free, contrary to an earlier assumption in this doc. The wrapper compensates by always passing `--disallowedTools` denying the dangerous built-ins (Bash, Read, Write, Edit, NotebookEdit, WebFetch, WebSearch, Glob, Grep, Task, KillShell), fail-closed, regardless of whether MCP wiring succeeded. When MCP is wired (the default; see [MCP tool access](#mcp-tool-access-no-setup-step-needed)), the task additionally passes `--allowedTools mcp__dicode` so Claude can call dicode's governed tool surface; setting `enable_mcp: false` drops that grant, but the built-in denylist still applies either way. As a subprocess, `claude` is still not confined by dicode's Deno sandbox, so the `run: ["claude"]` permission still understates what the binary itself can do at the OS level — but it can no longer reach host filesystem/bash/network tools through Claude's own tool-call interface. The MCP surface has grown to cover more governed authoring tools: dicode-core [#746](https://github.com/dicode-ayo/dicode-core/pull/746) turned every capability an authoring preset declares under `permissions.dicode` into a real, capability-gated tool — testing (`dicode_test_task`) and commit/PR (`dicode_git_commit_push`) among them — and [#738](https://github.com/dicode-ayo/dicode-core/pull/738) added `buildin/write-task-file`, letting `buildin/task-create` actually write a task's files to disk, scoped to the AI-scratch source's own task directories rather than an arbitrary dev-clone path. Deliberately still out of scope: secrets, crypto, audit, and the run-input retention sweep's own admin tools (`list_expired` / `delete_input`) stay off the model's toolset entirely — no `dicode_*` MCP tool exists for them — those belong to the task process, not the model driving it. (`dicode_pin_run_input`/`dicode_unpin_run_input` are different: they're real, capability-gated tools that *protect* a run's input from that same sweep, not sweep tools themselves.) Per-session sandbox confinement (constraining a write to *this session's* task directory rather than any task under the scratch source) also remains open, per #738's own "What was deliberately left behind" notes.
