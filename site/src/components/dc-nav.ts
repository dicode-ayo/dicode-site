...
export class DcNav extends LitElement {
  ...
  private _onScroll() {
    ...
    if (window.scrollY > 40) {
      nav.style.background = "var(--dicode-nav-bg-scrolled)";
    } else {
      nav.style.background = "var(--dicode-nav-bg)";
    }
  }
    ...
    return html`
      <style>
        dc-nav nav {
          ...
          background: var(--dicode-nav-bg); backdrop-filter: blur(16px);
          border-bottom: 1px solid var(--dicode-border); transition: background .3s;
        }
        dc-nav .nav-logo { font-size: 1.2rem; font-weight: 700; color: var(--dicode-heading); text-decoration: none; display: flex; align-items: center; gap: .5rem; }
        dc-nav .nav-logo span { color: var(--dicode-blue); }
        ...
        dc-nav .nav-links-desktop a {
          ...
          font-size: .85rem;
          transition: color var(--dicode-duration-fast);
          white-space: nowrap;
        }
        dc-nav .nav-links-desktop a:hover { color: var(--dicode-sky); }
        ...
        dc-nav .nav-mobile-menu {
          ...
          padding: var(--dicode-space-xl) var(--dicode-space-md);
          z-index: var(--dicode-z-nav);
          overflow-y: auto;
          border-top: 1px solid var(--dicode-border);
        }
        ...
        dc-nav .nav-mobile-menu a {
          ...
          transition: background var(--dicode-duration-fast) var(--dicode-ease),
        ...
        dc-nav .nav-github {
          ...
          transition: background var(--dicode-duration-fast) var(--dicode-ease),
                      border-color var(--dicode-duration-fast) var(--dicode-ease),
                      transform var(--dicode-duration-fast) var(--dicode-ease);
        }
        ...
        dc-nav .nav-right { display: flex; align-items: center; gap: var(--dicode-space-sm); }
        dc-nav .nav-hamburger {
          display: none; background: none; border: none; cursor: pointer;
          flex-direction: column; gap: 5px; padding: 4px; z-index: 110;
        }
        dc-nav .nav-hamburger span {
          display: block; width: 22px; height: 2px; background: var(--dicode-text);
